...
這是一條**零相依**的資料管線（YouTube 零金鑰；Twitch 需 Actions Secrets `TWITCH_CLIENT_ID`／`TWITCH_CLIENT_SECRET`，缺則 Twitch 組略過、其餘照常）:`fetch.mjs` 把 Google 表單 CSV 的頻道清單變成 `channels.json`,
...
- 網路相依集中在 `httpGet` / `resolveChannelId` / `fetchRss` / `fetchLive`(YouTube)與
  `fetchTwitchAppToken` / `fetchTwitchUsers` / `fetchTwitchStreams` / `fetchTwitchVideos`(Twitch);
  彙整邏輯在 `collectChannels`,這些抓取函式都可注入(`twitchToken`/`twitchUsers`/`twitchStreams`/
  `twitchVideos` 選項),新邏輯請用注入方式在 `test.mjs` 離線測(不要在測試裡打真網路)。
- Twitch 前置作業(`prepareTwitchContext`:拿 token → 批次查 users → 批次查 streams)一輪只跑一次,
  不是每個 Twitch 頻道各打一輪;缺 `TWITCH_CLIENT_ID`/`TWITCH_CLIENT_SECRET` 時完全不打 Twitch 端點。
