...
import { parseCsv, dedupeByGroup, dropTestRows, parseChannelRef, parseRss, parseStreamsPage, resolveChannelId, collectChannels, mapLimit } from './fetch.mjs';
...
await test('正式清單濾掉「測試｜」開頭的列，其餘原樣保留', () => {
  const rows = [
    { group: '測試｜三立新聞LIVE', url: 'https://www.youtube.com/channel/UC2TuODJhC03pLgd6MpWP0iw' },
    { group: '喂組', url: 'https://www.twitch.tv/wshixuan' },
    { group: '我們是測試｜組', url: 'https://www.twitch.tv/abc' },
  ];
  assert.deepEqual(dropTestRows(rows).map((r) => r.group), ['喂組', '我們是測試｜組']);
  assert.deepEqual(dropTestRows([]), []);
});
